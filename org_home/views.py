...
def IndexView(request):
    return render(request, 'org_home/index.html',{'member_categories_list': Categories.objects.filter(
...
def CategoryView(request):
    return render(request,'org_home/categories.html',{'categories_list':Categories.objects.all()})

# org_home page of specific category/root/branch in a coop
@login_required
def IndividualCategoryView(request,category_id):
    category = get_object_or_404(Categories,pk=category_id)
    return render(request,'org_home/individualCategory.html',{'category': category})
...
def submitNewCategory(request):
    if 'new_category' in request.POST and request.POST['new_category'] != '':
        ...
        if Categories.objects.filter(category_name=formatedCategoryName).exists():
            return render(request, 'org_home/newCategory.html', {'error_message': formatedCategoryName + " already exists.",})
        ...
        return HttpResponseRedirect(reverse('org_home:categories'))
    else:
        return render(request, 'org_home/newCategory.html', {
...
def membersView(request,category_id):
    category = get_object_or_404(Categories,pk=category_id)
    return render(request,'org_home/members.html',{'category': category})
...
def pendingMembersView(request,category_id):
    ...
    category = get_object_or_404(Categories,pk=category_id)
    return render(request,'org_home/pendingMembers.html',{'category': category})
...
def GrantAccess(request,category_id,pending_member_id):
    ...
    # if only moderator can add pending members to category
        ...
        if request.user in category.moderators.all():
            ...
            return render(request,'org_home/pendingMembers.html',{'category': category,'error_message':'Must be a moderator to add user to ' +  category.category_name })
    return HttpResponseRedirect(reverse('org_home:pendingMembersView', args=(category.id,)))
